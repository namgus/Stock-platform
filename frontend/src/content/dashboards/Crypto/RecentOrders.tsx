...
function RecentOrders() {
  ...
  useEffect(() => {
    ...
    const apiCalls = Object.keys(dictionary).map((key) => {
      ...
      return axios.all([
        axios.get(`http://43.200.2.113:8000/stock/financial?code=${companyCode}`),
        axios.get(`http://43.200.2.113:8000/stock/price?code=${companyCode}`),
        axios.get(`http://43.200.2.113:8000/stock/predict?code=${companyCode}`)
      ]);
    });
